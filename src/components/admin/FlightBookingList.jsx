...
import { flightBookingListHeader } from "../utility/MenuItems";
...
export default function DataTable() {
  ...
  const departureSuccess = rows.filter(
    (row) => row.departureStatus === "Success"
  ).length;
  const departureFailure = rows.filter(
    (row) => row.departureStatus === "Failed"
  ).length;
  const returnSuccess = rows.filter(
    (row) => row.returnStatus === "Success"
  ).length;
  const returnFailure = rows.filter(
    (row) => row.returnStatus === "Failed"
  ).length;
